...
import { buildUpstreamHeaders, createProxyConfig, proxyRequest, validateProxyPath } from './proxy-core.js';

const config = createProxyConfig({ LOP_SACH_API_ORIGIN: 'http://127.0.0.1:3000', LOP_SACH_PROXY_SECRET: 'a'.repeat(32), NODE_ENV: 'test' });
...
describe('fixed upstream proxy', () => {
  ...
  it('forwards allowlisted headers and sanitized platform IP only', () => {
    const headers = buildUpstreamHeaders(new Headers({ cookie: 'session=x', origin: 'https://app.test', authorization: 'Bearer no', connection: 'upgrade', 'x-request-id': 'request-123', 'x-vercel-forwarded-for': '203.0.113.7, bad' }), config);
  ...
  it('preserves method, query and body without accepting an alternate target', async () => {
    ...
    expect(String(target)).toBe('http://127.0.0.1:3000/api/v1/auth/login?continue=https%3A%2F%2Fattacker.invalid');
  ...
  it('rejects bodies over the route-specific limit before contacting upstream', async () => {
    ...
    }, config, fetcher)).rejects.toThrow('PROXY_BODY_TOO_LARGE');
  ...
  it('preserves separate cookies and forces no-store', async () => {
    ...
    const fetcher = vi.fn(() => Promise.resolve(new Response('{}', { status: 200, headers: responseHeaders })));
    const result = await proxyRequest({ method: 'GET', pathAndQuery: '/api/v1/auth/me', headers: new Headers() }, config, fetcher);
