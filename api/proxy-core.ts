...
const HOP_BY_HOP = new Set(['connection', 'keep-alive', 'proxy-authenticate', 'proxy-authorization', 'te', 'trailer', 'transfer-encoding', 'upgrade', 'host', 'authorization']);
const FORWARDED_HEADERS = new Set(['accept', 'content-type', 'cookie', 'origin', 'if-match', 'x-request-id']);

export interface ProxyConfig { readonly upstream: URL; readonly secret: string }
export interface ProxyInput { readonly method: string; readonly pathAndQuery: string; readonly headers: Headers; readonly body?: Uint8Array }
export interface ProxyOutput { readonly status: number; readonly headers: Headers; readonly setCookies: readonly string[]; readonly body: Uint8Array }
...
export function createProxyConfig(environment: NodeJS.ProcessEnv): ProxyConfig {
  ...
  if (!rawOrigin || !secret || secret.length < 32) throw new Error('Proxy cần fixed upstream và secret tối thiểu 32 ký tự.');
  const upstream = new URL(rawOrigin);
  if (upstream.protocol !== 'https:' && environment.NODE_ENV === 'production') throw new Error('Production upstream phải dùng HTTPS.');
  if (upstream.username || upstream.password || upstream.pathname !== '/' || upstream.search || upstream.hash) throw new Error('Upstream phải là một origin cố định, không chứa credential/path/query/hash.');
...
export function validateProxyPath(pathAndQuery: string): string {
  ...
  if (parsed.origin !== 'https://proxy.invalid' || !/^\/api\/v1(?:\/|$)/u.test(parsed.pathname)) throw new Error('PROXY_ROUTE_NOT_ALLOWED');
...
export async function proxyRequest(input: ProxyInput, config: ProxyConfig, fetcher: typeof fetch = fetch): Promise<ProxyOutput> {
  ...
  try {
    ...
    for (const [name, value] of response.headers.entries()) {
      if (!HOP_BY_HOP.has(name.toLowerCase()) && name.toLowerCase() !== 'set-cookie') headers.set(name, value);
    }
    headers.set('cache-control', 'no-store');
    return { status: response.status, headers, setCookies: responseCookies(response.headers), body: new Uint8Array(await response.arrayBuffer()) };
