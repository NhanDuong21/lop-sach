...
export default async function handler(request: IncomingMessage, response: ServerResponse): Promise<void> {
  try {
    ...
    const body = request.method === 'GET' || request.method === 'HEAD' ? undefined : await readBody(request, backup ? 2 * 1024 * 1024 : 256 * 1024);
    const result = await proxyRequest({ method: request.method ?? 'GET', pathAndQuery: request.url ?? '/', headers, ...(body ? { body } : {}) }, config);
    ...
    response.statusCode = message === 'PROXY_ROUTE_NOT_ALLOWED' ? 404 : message === 'PROXY_BODY_TOO_LARGE' ? 413 : 502;
    response.setHeader('Content-Type', 'application/problem+json');
    response.setHeader('Cache-Control', 'no-store');
    response.end(JSON.stringify({ code: message, detail: 'Không thể chuyển tiếp request tới API.' }));
  }
}
