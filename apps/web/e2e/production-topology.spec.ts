...
test.describe('@topology production chain', () => {
  ...
  test('verifies HTTPS, session persistence, origin guard, health and cache policy', async ({ context, page, request }) => {
    ...
    const meResponsePromise = page.waitForResponse((response) => response.url().includes('/api/v1/auth/me'));
    await page.reload();
    ...
    await expect(page.getByRole('heading', { name: 'Đăng nhập để phân công trực nhật' })).toBeVisible();
    expect((await context.cookies(webOrigin as string)).some((cookie) => cookie.name === '__Host-lop_sach_session')).toBe(false);
