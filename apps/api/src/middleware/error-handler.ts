...
export const errorHandler: ErrorRequestHandler = (
  ...
  if (error instanceof ZodError) {
    response.status(422).json({
      ...problemDetails(
        new HttpProblem(422, 'VALIDATION_FAILED', 'Dữ liệu không hợp lệ.'),
        request.originalUrl,
        requestId,
      ),
  ...
  const expectedPersistenceConflict =
  ...
  if (!(error instanceof HttpProblem) && !expectedPersistenceConflict)
