...
export function createClassroomRouter(): Router {
  ...
  router.get('/', async (_request, response, next) => {
    try {
      response.json({ data: await getClassroom(authenticatedUser(response).id) });
    } catch (error) {
      next(error);
    }
  });
  router.put('/', async (request, response, next) => {
    try {
      response.status(201).json({
        data: await createClassroom(
          authenticatedUser(response).id,
          ClassroomUpsertSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.patch('/', async (request, response, next) => {
    try {
      response.json({
        data: await patchClassroom(
          authenticatedUser(response).id,
          ClassroomPatchSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/groups', async (request, response, next) => {
    try {
      ...
      response.status(201).json({
        data: await createGroup(authenticatedUser(response).id, input.name, input.expectedVersion),
      });
    } catch (error) {
      next(error);
    }
  ...
  router.patch('/groups/:groupId', async (request, response, next) => {
    try {
      response.json({
        data: await patchGroup(
          authenticatedUser(response).id,
          request.params.groupId ?? '',
          GroupPatchSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
