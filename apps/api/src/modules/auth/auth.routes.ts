...
async function classroomStatus(userId: string): Promise<{ readonly hasClassroom: boolean; readonly onboardingCompleted: boolean }> {
  const classroom = await ClassroomModel.findOne({ ownerId: userId }).select({ 'onboarding.completedAt': 1 }).lean();
  return { hasClassroom: Boolean(classroom), onboardingCompleted: Boolean(classroom?.onboarding?.completedAt) };
}

export function createAuthRouter(config: AppConfig, options: { readonly rateLimits?: boolean } = {}): Router {
  const router = Router();
  const cookie = sessionCookiePolicy(config.environment);
  const requireAuth = authenticate(cookie);

  router.post('/login', ...(options.rateLimits === false ? [] : [loginRateLimit]), async (request, response, next) => {
    try {
      ...
      response.status(200).json({ data: { id: String(user._id), displayName: user.displayName, username: user.username, ...status } });
