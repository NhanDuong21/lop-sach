import {
  ...
  TaskTemplateOrderSchema,
  TaskTemplatePatchSchema,
...
export function createTaskTemplateRouter(): Router {
  ...
  router.get('/', async (_request, response, next) => {
    try {
      response.json({ data: await listTaskTemplates(authenticatedUser(response).id) });
    } catch (error) {
      next(error);
    }
  });
  router.post('/', async (request, response, next) => {
    try {
      response.status(201).json({
        data: await createTaskTemplate(
          authenticatedUser(response).id,
          TaskTemplateCreateSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.patch('/:taskId', async (request, response, next) => {
    try {
      response.json({
        data: await patchTaskTemplate(
          authenticatedUser(response).id,
          request.params.taskId ?? '',
          TaskTemplatePatchSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/:taskId/activate', async (request, response, next) => {
    try {
      ...
      response.json({
        data: await setTaskTemplateActive(
          authenticatedUser(response).id,
          request.params.taskId ?? '',
          true,
  ...
  router.put('/order', async (request, response, next) => {
    try {
      ...
      response.json({
        data: await reorderTaskTemplates(
          authenticatedUser(response).id,
          input.taskIds,
          input.expectedTasksRevision,
        ),
      });
    } catch (error) {
      next(error);
