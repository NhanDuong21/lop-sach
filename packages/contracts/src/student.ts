...
const NoHeavyTasksRestrictionWriteSchema = RestrictionWriteBaseSchema.extend({ type: z.literal('NO_HEAVY_TASKS') });
const TaskExclusionRestrictionWriteSchema = RestrictionWriteBaseSchema.extend({ type: z.literal('TASK_EXCLUSION'), taskTemplateId: z.string().min(1) });
const ExemptDateRangeRestrictionWriteSchema = RestrictionWriteBaseSchema.extend({
  type: z.literal('EXEMPT_DATE_RANGE'), startDate: DateOnlySchema, endDate: DateOnlySchema,
}).refine((value) => value.endDate >= value.startDate, { message: 'Ngày kết thúc miễn phải từ ngày bắt đầu trở đi.', path: ['endDate'] });
export const StudentRestrictionWriteSchema = z.discriminatedUnion('type', [NoHeavyTasksRestrictionWriteSchema, TaskExclusionRestrictionWriteSchema, ExemptDateRangeRestrictionWriteSchema]);
...
function validateParticipationRange(value: { readonly participationStart?: string | null | undefined; readonly participationEnd?: string | null | undefined }, context: z.RefinementCtx): void {
  if (typeof value.participationStart === 'string' && typeof value.participationEnd === 'string' && value.participationEnd < value.participationStart) {
...
export const StudentCreateSchema = StudentWriteFieldsSchema.extend({
  groupId: z.string().min(1), active: z.boolean().default(true),
}).superRefine(validateParticipationRange);
export const StudentPatchSchema = StudentWriteFieldsSchema.partial().extend({
  expectedVersion: z.number().int().nonnegative(),
}).superRefine(validateParticipationRange);
export const StudentMoveSchema = z.strictObject({ groupId: z.string().min(1), expectedVersion: z.number().int().nonnegative() });
